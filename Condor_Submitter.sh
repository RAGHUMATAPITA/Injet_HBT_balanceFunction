...
if [ $running_system == 1 ]; then
    #file="Data_MC_txtFiles/PbPb_2018_AODMC_New_Vanderbilt_WithTracks.txt"
    file="Data_MC_txtFiles/PbPb_2018_AODMC_New_Vanderbilt_WithTracks_new.txt"
    echo ${file}
elif [ $running_system == 2 ]; then
    file="Data_MC_txtFiles/Pythia_pThat15.txt"
    echo ${file}
elif [ $running_system == 3 ]; then
    file="Data_MC_txtFiles/PbPb_2018_AODData_New_Vanderbilt_WithTracks.txt"
    echo ${file}
...
if [ $istestrun == 1 ]; then
    if [ $isMC == 1 ]; then
	root -l -b -q "file_write_copy.C(${testRun_condition}, \"${file}\", \"$1\", \"espresso\", 4, \"/eos/user/r/rpradhan/Research/AT_UIC_IITM/RootFiles_Folder/MC\", \"$2\", \"$3\", ${mc_or_data})"
    else
	root -l -b -q "file_write_copy.C(${testRun_condition}, \"${file}\", \"$1\", \"espresso\", 4, \"/eos/user/r/rpradhan/Research/AT_UIC_IITM/RootFiles_Folder/Data\", \"$2\", \"$3\", ${mc_or_data})"
    fi
else
    if [ $isMC == 1 ]; then
	root -l -b -q "file_write_copy.C(${testRun_condition}, \"${file}\", \"$1\", \"tomorrow\", 4, \"/eos/user/r/rpradhan/Research/AT_UIC_IITM/RootFiles_Folder/MC\", \"$2\", \"$3\", ${mc_or_data})"
    else 
	root -l -b -q "file_write_copy.C(${testRun_condition}, \"${file}\", \"$1\", \"tomorrow\", 4, \"/eos/user/r/rpradhan/Research/AT_UIC_IITM/RootFiles_Folder/Data\", \"$2\", \"$3\", ${mc_or_data})"
    fi
fi
