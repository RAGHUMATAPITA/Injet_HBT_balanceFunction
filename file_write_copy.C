...
void file_write_copy(bool isTestRun = true, string inputfile = "Data_MC_txtFiles/Hydjet_pThat15.txt", string outfile = "Hydjet_pThat15", string jobflavour= "workday", int nCpus = 4, string save_outrootfile_folder = "/eos/user/r/rpradhan/Research/AT_UIC_IITM/RootFiles_Folder/MC/Pythia_pThat15", string diff_folder = "pThat30_pTCut40_NoJetTrigger", string colliding_system = "PbPb", int ismc = 1)
//void file_write()
{
  std::cout<<std::endl;
  ...
  if(ratio != file_name_vector.size() || ratio == file_name_vector.size())
      ...
      for(int i = 0; i < njobs; i++)
	{
      ...
      //myfile_exe<<"cd /afs/cern.ch/user/r/rpradhan/CMSSW_10_3_1_patch2/src"<<std::endl;
      //myfile_exe<<"eval `scramv1 runtime -sh`"<<std::endl;
      myfile_exe<<"cd /afs/cern.ch/work/r/rpradhan/AT_UIC_IITM/TestCode_HBT"<<std::endl;
